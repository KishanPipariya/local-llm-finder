...
4. Manually review each displayed installation path. Every card must link to its Hugging Face source. Ollama GGUF cards must use the displayed `curl` plus `ollama create` import recipe, never `ollama pull`; LM Studio and llama.cpp cards retain their Hugging Face file guidance, and MLX cards their MLX guidance. This is a command-and-URL review only: no model download or execution is claimed.
