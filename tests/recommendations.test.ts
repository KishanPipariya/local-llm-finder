...
test("validates configuration boundaries", () => { assert.equal(validateConfig(mac).valid, true); assert.equal(validateConfig({ ...mac, memoryGb: 7 }).valid, false); assert.equal(validateConfig({ ...mac, diskGb: 0 }).valid, false); });
test("excludes MLX on Intel and flags Intel performance", () => { const intel = { ...mac, architecture: "intel" as const, macosMajor: 13 }; assert.deepEqual(runtimeEligibility(intel, mlx), []); const result = rankArtifacts([gguf], intel)[0]; assert.ok(result.notes.some((note) => note.includes("CPU-only"))); assert.ok(!result.runtimes.includes("Ollama")); });
