...
export function RecommendationCard({ model }: { model: Recommendation }) {
  ...
  return <article className="card" aria-labelledby={headingId}>
    ...
    <section className="recipe" aria-label={`Setup recipe for ${model.title}`}><h4>Setup recipe</h4>{model.guidance.map((guide) => <div className="guide" key={guide.runtime}><b>{guide.runtime}</b><code>{guide.command}</code></div>)}</section>
